...
const getRandomImage = () => {
  return mockProductImages[Math.floor(Math.random() * mockProductImages.length)];
};
...
export const transactionService = {
  async getTransactions(params?: GetTransactionsParams): Promise<FrontendTransaction[]> {
    try {
      ...
      const transactions: FrontendTransaction[] = data.transactions.map((transaction: BackendTransaction) => {
  ...
  async updateTransactionStatus(id: string, statusData: UpdateStatusData): Promise<FrontendTransaction> {
    try {
      const headers = await getAuthHeaders();
  ...
  async contactCustomerCare(id: string, contactData: ContactCustomerCareData): Promise<void> {
    try {
      const headers = await getAuthHeaders();
      
      const response = await fetch(`${API_BASE_URL}/api/transactions/${id}/contact-customer-care`, {
        method: 'POST',
        headers,
