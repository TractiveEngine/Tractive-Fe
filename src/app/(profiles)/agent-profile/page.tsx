...
// Default pill options — merged at runtime with any extras from the API
...
const ProfileSetting = () => {
  ...
  const [interestOptions, setInterestOptions] =
    useState<string[]>(DEFAULT_INTERESTS);

  // Populate form once profile data arrives
  useEffect(() => {
    if (!profile) return;

    const apiInterests: string[] = Array.isArray(profile.interests)
      ? profile.interests
      : [];

    // Append any API interests not in the default list (e.g. "fish")
    setInterestOptions((prev) => {
      const merged = [...prev];
      apiInterests.forEach((i) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Exact payload — email and image excluded
  ...
  return (
    <div className="w-[100%] bg-[#fefefe] flex flex-col items-center shadow-md rounded-[4px]">
      ...
      >
        ...
        {/* Interests */}
        <div className="w-full">
          <label className="font-montserrat font-normal text-[13px] text-[#2b2b2b] mb-2 block">
            Interests
          </label>
            ...
            {interestOptions.map((interest) => {
              ...
              return (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-[12px] font-montserrat border transition-colors ${
                    selected
                      ? "bg-[#538E53] text-[#fefefe] border-[#538E53]"
                      : "bg-[#fefefe] text-[#2b2b2b] border-[#e2e2e2] hover:border-[#538E53]"
                  }`}
                >
                  {interest}
                </button>
