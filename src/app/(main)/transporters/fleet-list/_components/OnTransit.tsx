...
export const OnTransit: React.FC = () => {
  ...
  const filteredFleets = useMemo(() => {
    return fleets.filter((fleet) => {
      const matchesStatus = fleet.status === "On transit";
  ...
  const handleToggleStatus = (id: string) => {
    setFleets(
      fleets.map((fleet) =>
        fleet.id === id ? { ...fleet, status: "Available" } : fleet
      )
    );
  };
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 w-[100%] sm:w-[90%] md:w-[80%] lg:w-[70%] xl:w-[60%] 2xl:w-[50%]">
            ...
            <div className="relative w-[100%] sm:w-[70%] flex-grow">
              <input
                ...
                aria-label="Search on transit fleets"
                aria-describedby="search-description"
              />
              <span id="search-description" className="sr-only">
                Search for on transit fleets by name or IOT
              </span>
