...
import { useTransporterRevenue } from "@/hooks/queries/useTransporterDashboardQueries";
...
const CustomTooltip = ({ active, payload }: TooltipProps<number, string>) => {
  if (active && payload && payload.length) {
    ...
    const label = parsed
      ? parsed.toLocaleString("en-US", { month: "short", year: "numeric" })
      : point.date;
    return (
...
export const TransporterRevenueChart = () => {
  const { data, isLoading, isError } = useTransporterRevenue({
    granularity: "month",
  });
  ...
  return (
    <div className="w-full bg-[#fefefe] rounded-[4px] shadow-md">
      <h2 className="font-montserrat text-[#2b2b2b] text-[13px] p-2 pl-4 rounded-tl-[6px] rounded-br-[6px]  font-medium mb-4 bg-[#cce5cc] flex items-center w-[25%]">
        Revenue
      </h2>
